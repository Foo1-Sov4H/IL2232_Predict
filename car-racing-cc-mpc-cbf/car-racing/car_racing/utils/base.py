...
class MPCCBFRacing(ControlBase):
    ...
    def calc_input(self):
        ...
        if self.realtime_flag == False:
           # print("time: ", self.time) # print current time, for chance constraint debugging
           # print("xcurv: ", self.x)
           # print("self.racing_sim.vehicles: ",self.racing_sim.vehicles)
            #print("xglobs: ",self.xglobs)

            #print("lap_xcurvs: ",self.lap_xcurvs)
            #print("self.lap_xglobs: ",self.lap_xglobs)
...
class DynamicBicycleModel(ModelBase):
    ...
    def forward_dynamics(self, realtime_flag):
        ...
        if ((realtime_flag == True) and (self.u is None)) or self.zero_noise_flag:
            ...
            xcurv_next[0] = xcurv_next[0] + 0.5 * noise_vx # the longitudinal velocity
            xcurv_next[1] = xcurv_next[1] + 0.5 * noise_vy # the lateral velocity
            xcurv_next[2] = xcurv_next[2] + 0.5 * noise_wz # yaw rate
